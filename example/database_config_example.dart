...
library database_config_example;
...
class PostgresConnectionFactory
    implements ConnectionFactory<MockPostgresConnection> {
  @override
  Future<MockPostgresConnection> createConnection(
      DatabaseConnection config) async {
    // In real implementation, this would create actual database connection
    print(
        'Creating PostgreSQL connection to ${config.host}:${config.port}/${config.database}');
    await Future.delayed(
        Duration(milliseconds: 100)); // Simulate connection time
...
class MockPostgresConnection {
  ...
  MockPostgresConnection(this.config)
...
Future<void> exampleProgrammaticConfiguration() async {
  ...
  print(
      'Configuration created with ${config.connectionNames.length} connections');
...
Future<void> exampleEnvironmentConfiguration() async {
  ...
  final envVars = {
    ...
    'DB_ANALYTICS_URL':
        'postgresql://analytics:secret@analytics.internal:5432/warehouse',
  };
...
Future<void> exampleConnectionPooling() async {
  ...
  print(
      'Pool initialized with ${pool.getStatistics()['total_connections']} connections');
